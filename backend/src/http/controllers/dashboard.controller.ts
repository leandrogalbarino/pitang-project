...
export const getDashboardStats = async (request: Request, response: Response) => {
  try {
    ...
    const stats = {
      ...
      totalAmountPaid: statusCounts
        ...
        .reduce((acc, curr) => acc + (curr._sum.amount || 0), 0),
      pendingApproval: statusCounts.find(s => s.status === 'ENVIADO')?._count || 0,
      pendingPayment: statusCounts.find(s => s.status === 'APROVADO')?._count || 0,
